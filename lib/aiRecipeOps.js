...
3. Hands-on/attended steps DO get timers when the duration is stated and a timer genuinely helps (e.g., "boil pasta for 10 min", "sear 3 min each side", "sauté for 5 min") — label their involvement "active" so the cook knows they must stay involved.
...
13. Every timer gets an "involvement" level telling the cook how much attention it needs:
    - "none": the cook can walk away and come back when it rings. Fully unattended passive waits (baking, roasting covered, simmering covered, resting, proofing, chilling, marinating, steeping).
    - "low": the cook should stay in the kitchen and check in every so often (boiling - watch for boil-over, stirring occasionally, reducing, flipping halfway, uncovered simmering, basting).
    - "active": the cook must stay involved the whole time (sautéing, searing, frying, stir-frying, whisking, anything needing constant attention).
    Checkpoints do not need involvement (they inherit the parent timer's).
...
Example 6 - Attended (active) timer:
Instructions: "1. Boil pasta for 10 min. 2. Cook the sauce."
→ timer-1: boil pasta, duration 10, involvement "active", deps: [{timerId: "start", offset: 0}]
...
Example 8 - Involvement levels:
Instructions: "1. Bake the casserole for 40 min. 2. Simmer the sauce uncovered for 25 min, stirring every few minutes. 3. Fry the halloumi for 4 min."
→ timer-1: bake casserole, duration 40, involvement "none", deps: [{timerId: "start", offset: 0}]
→ timer-2: simmer sauce, duration 25, involvement "low", deps: [{timerId: "start", offset: 0}] (parallel)
→ timer-3: fry halloumi, duration 4, involvement "active", deps: [{timerId: "start", offset: 0}] (short stated duration, cook is involved)
...
export function parseTimerResult(data) {
    const result = {};
    const INVOLVEMENT_LEVELS = ['none', 'low', 'active'];
    ...
    if (Array.isArray(data.timers)) {
        result.timers = data.timers
            ...
            .map(item => ({
                ...
                involvement: INVOLVEMENT_LEVELS.includes(item.involvement) ? item.involvement : 'active',
