...
function extractIngredientName(inputText) {
    let ingredName = inputText.trim();

    const numberFound = extractQuantities(inputText);

    if (numberFound.length > 0) {
        // Remove quantities
        ingredName = ingredName.replace(numberFound.join(' '), '');
    }

    // Remove quantities in parentheses
    const parenthesesMatch = ingredName.match(/\([^()]+\)/g);
    if (parenthesesMatch) {
        ingredName = ingredName.replace(parenthesesMatch.join(' '), '');
    }

    // Remove split measures
    const splitMeasureMatch = ingredName.match(/\d+\s*\+\s*\d+\/\d+/g);
    if (splitMeasureMatch) {
        ingredName = ingredName.replace(splitMeasureMatch.join(' '), '');
    }

    // Remove fractions
    const fractionMatch = ingredName.match(/\d+\s*\/\s*\d+/g);
    if (fractionMatch) {
        ingredName = ingredName.replace(fractionMatch.join(' '), '');
    }

    // Split by comma and take the first part as the ingredient name
    ingredName = ingredName.split(",")[0].trim();

    // If nameFound is present, use its value instead
    if (nameFound) {
