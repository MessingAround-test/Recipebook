...
export function convertMetricReading(inputText) {
    ...
    for (const index in wordsFound) {
        ...
        if (keyFound) {
            return { "quantity": numberFound, "quantity_unit": keyFound, "quantity_type": quantity_unit_conversions[keyFound].type }
        }
    }

    return { "quantity": numberFound, "quantity_type": "each", "quantity_unit": "each"}
}
