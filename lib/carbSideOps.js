...
export function computeCarbInsertPoint(instructions, totalMinutes, hintIndex) {
    ...
    if (n === 0) return 0
    const times = steps.map(s => (s && typeof s.time === 'number' && s.time > 0) ? s.time : null)
    const known = times.filter(t => typeof t === 'number')
    ...
    for (let i = n - 1; i >= 0; i--) {
        ...
        // Reached the start of the recipe and still below total — start the
        // carb at the beginning so it still finishes with the last step.
        if (i === 0) insertIdx = 0
    }

    if (typeof hintIndex === 'number' && hintIndex >= 0 && hintIndex <= n && Math.abs(hintIndex - insertIdx) <= 1) {
        insertIdx = hintIndex
    }
    return insertIdx
}

/** Core backward walk: insert 0-based index for a step that must START
 *  `minutesFromEnd` before the recipe's last step finishes. */
function insertIndexForMinutesFromEnd(instructions, minutesFromEnd) {
    const steps = instructions || []
    const n = steps.length
    if (n === 0) return 0
    const need = (typeof minutesFromEnd === 'number' && minutesFromEnd > 0) ? minutesFromEnd : 0
    if (need === 0) return n - 1 // rides the very last step
    const times = steps.map(s => (s && typeof s.time === 'number' && s.time > 0) ? s.time : null)
    const known = times.filter(t => typeof t === 'number')
    const fallback = known.length > 0 ? known.reduce((a, b) => a + b, 0) / known.length : 10
    const resolved = times.map(t => (typeof t === 'number' ? t : fallback))
 ...
 *  Returns a new phases array, each carrying `insertAfter` (0-based). */
export function computePhaseInsertPoints(instructions, phases) {
    ...
    return list.map((p, i) => {
        ...
        return {
            ...p,
            insertAfter: insertIndexForMinutesFromEnd(instructions, own)
        }
    })
}
