...
export default async function handler(req, res) {
    if (req.method === "GET") {
        ...
        const regexPattern = /<span class="wprm-recipe-ingredient-amount">([^<]+)<\/span>\s*(?:<span class="wprm-recipe-ingredient-unit">(.*?)<\/span>\s*)?<span class="wprm-recipe-ingredient-name">(.*?)<\/span>/g;
        ...
        const ingredientAmountRegex = /<span class="wprm-recipe-ingredient-amount[^>]*?">(.*?)<\/span>/gs;
        const ingredientNameRegex = /<span class="wprm-recipe-ingredient-name[^>]*?">(.*?)<\/span>/gs;
        ...
        matches.forEach((match, index) => {
            const quantity =  `${match[1].trim()}${match[2]?match[2]:""}`.trim();
            const name = match[3].trim();

            const rawIngredient = quantity + " " + name; // Value of data-raw-ingredient attribute
            let converted;
            try {
        ...
        const instructionPattern = /<div class="wprm-recipe-instruction-text[^>]*?">(.*?)<\/div>/gs;
